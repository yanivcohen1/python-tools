...
solution_2 = odeint(system_of_odes, initial_conditions_2, time_points, args=(m1_val, m2_val, g_val))
...
# Extract position and velocity from the solution_2
the1_sol_2 = solution_2[:, 0]
the1_d_sol_2 = solution_2[:, 1]

the2_sol_2 = solution_2[:, 2]
the2_d_sol_2 = solution_2[:, 3]

x1_pendulum_2 = np.sin(the1_sol_2)
y1_pendulum_2 = -np.cos(the1_sol_2)

x2_pendulum_2 = x1_pendulum_2 + np.sin(the2_sol_2)
...
def update(frame):
    ...
    # pandilum 2
    pendulum1_2.set_data([0, x1_pendulum_2[frame]], [0, y1_pendulum_2[frame]])
    mass1_2.set_data([x1_pendulum_2[frame]], [y1_pendulum_2[frame]])

    pendulum2_2.set_data([x1_pendulum_2[frame], x2_pendulum_2[frame]], [y1_pendulum_2[frame], y2_pendulum_2[frame]])
    mass2_2.set_data([x2_pendulum_2[frame]], [y2_pendulum_2[frame]])

    return pendulum1, mass1, pendulum2, mass2,  pendulum1_2, mass1_2, pendulum2_2, mass2_2
...
# pandilum 2
pendulum1_2, = ax.plot([0, x1_pendulum[0]], [0, y1_pendulum[0]], lw=2)
mass1_2, = ax.plot([x1_pendulum[0]], [y1_pendulum[0]], 'o', markersize=4*int(m1_val)+1, color='blue')

pendulum2_2, = ax.plot([x1_pendulum[0], x2_pendulum[0]], [y1_pendulum[0], y2_pendulum[0]], lw=2)
