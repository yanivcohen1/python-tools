...
# start gui
st.write("### PID control simulation")

col1, col2, col3 = st.columns(3)

P = col1.slider('P', value=0.1, min_value=-0.2, max_value=1.0, step=0.05)
...
if "init" not in st.session_state:
    ...
    st.session_state["fig"] = make_subplots(rows=2, cols=2) #, subplot_titles=("Plot 1", "Plot 2", "Plot 3", "Plot 4"))
    ...
    st.session_state["t"] = np.linspace(0,tf,n) # create time vector
    t = st.session_state["t"]
    # plot PID response
    # plot 1,1
