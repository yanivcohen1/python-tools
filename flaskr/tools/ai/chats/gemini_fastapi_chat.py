...
genai.configure(api_key="AIzaSyBbTr3sE75zbdCybGHbGVe_nP_XUhmE-Xk") #replace with your api key
...
def get_weather(city: str) -> str:
    """
    This function get the weather for a given city
    ...
    url = "https://api.weatherapi.com/v1/current.json?key=c2381724f089458c810180321252907&q="+ city # London
    response = requests.get(url, timeout=10).json()
    return f"{response["current"]["condition"]["text"]}, {response["current"]["temp_c"]}°C" # "Sunny, 25°C"

# list me the tools you have
# calculate 14+4 and send the result to the user
tools = [greeting, send_user, calculator, get_weather] # Using a model expected to support tool use
